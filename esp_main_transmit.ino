...
class MyDataCallbacks : public BLECharacteristicCallbacks {
    void onWrite(BLECharacteristic *pCharacteristic) {
        ...
        if (value.length() > 0) {
            ...
            // Если вы хотите дополнительно обработать полученные данные,
            // вы можете парсить строку `userData` и извлекать значения.
        }
    }
};
...
void saveUserData(const std::string &data) {
    preferences.begin("storage", false); // Открываем NVS для записи
    preferences.putString("userData", data.c_str()); // Сохраняем строку
    preferences.end(); // Закрываем NVS
    Serial.println("Data saved to NVS.");
}

std::string loadUserData() {
    preferences.begin("storage", true); // Открываем NVS для чтения
    String savedData = preferences.getString("userData", ""); // Загружаем строку
    preferences.end(); // Закрываем NVS
    if (savedData.length() > 0) {
        Serial.println("Data loaded from NVS: " + savedData);
        return std::string(savedData.c_str());
    }
